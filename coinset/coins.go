...
func (cs *CoinSet) TotalValue() (value cashutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue cashutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue cashutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount cashutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue cashutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue cashutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue cashutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue cashutil.Amount, coins []Coin) (Coins, error) {
...
// cashutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() cashutil.Amount {
	return cashutil.Amount(c.txOut().Value)
}
